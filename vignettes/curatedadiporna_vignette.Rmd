...
* The metadata `metadata` which contain two objects `qc` a list of `qc_read` 
objects of each `fastq` file and a `data.frame` of the studies from which the
...
The metadata is a list of two objects. First is `qc`; a `list` of items that 
correspond to each `fastq` file. Each item is a `list`/`qc_read`. The second is
`data.frame` containing the metadata of the studies from which the data were 
collected. Here we show the first entry in `bib` format.

```{r metadata, message=FALSE}
# print class of first sample
class(metadata(adipo_counts)$qc[[1]])

# print data of first study
df2bib(metadata(adipo_counts)$studies[1,])
```
...
file. In this case because some of the samples are paired-end, so each of them would have two files `SRR\*_1` and `SRR\*_2`. This kind of information is 
available in `colData`. Next, we subset the `qc` object of the metadata with 
the identified run ids. 
...
# get runs ids
single_runs <- se$run[se$library_layout == 'SINGLE']
paired_runs <- paste(se$run[se$library_layout == 'PAIRED'], 1:2, sep = '_')
length(c(single_runs, paired_runs))

# get qc data for each run
qc <- metadata(se)$qc
qc <- qc[c(single_runs, paired_runs)]

# check all runs has qc
all(c(single_runs, paired_runs) %in% names(qc))
```

The `qc` object of the metadata contains the output of [fastqc](https://www.bioinformatics.babraham.ac.uk/projects/fastqc/) 
...
per_base <- lapply(as.list(qc), function(x) {
...
The run IDs of the "bad" samples is then used to remove them form the dataset.

```{r remove_lowscore}
# get run ids of low quality samples
bad_samples <- unique(per_base$run[per_base$run_average == FALSE])

# subset the counts object
se2 <- se[, !se$run %in% bad_samples]
table(se2$time)
```

## Filtering low count genes

To identify the low count feature/genes (possible not expressed), we keep only the
...
## Citting the studies in this subset of the data
...
studies <- metadata(se3)$studies
studies$BIBTEXKEY[studies$BIBTEXKEY %in% se3$bibtexkey]
```
