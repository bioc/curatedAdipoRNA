...
* In case the experimental designs included treatment other than the differentiation medias, the control (non-treated) samples were included.
...
The metadata is a list of two objects. First is `qc`; a `list` of items that correspond to each `fastq` file. Each item is a `list`/`qc_rea`. The second is `data.frame` containing the metadata of the studies from which the data were collected. Here we show the first entry in `bib` format.
