...
`inst/scripts/`, the processed data in `inst/extdata/` and the final 
`RangedSummarizedExperiment` object in `data/`.
...
The `RangedSummarizedExperiment` object contains the `gene_counts`, `colData`,
...
The source code of the package is hosted on GitHub and can be installed using 
`devtools`.
...
## Exploring the `gene_counts` object
...
correspond to each `fastq` file. Each item is a `list`/`qc_rea`. The second is
